...
internal fun PluginSearchSortControls(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        FlowRow(
                    ...
                    Box(modifier = Modifier.height(360.dp)) {
                        Column(
                            ...
                            pageFilterOptions.forEach { option ->
                                val selected = option.id in selectedPageFilterIds
                                DropdownMenuItem(
                                    text = { Text("${if (selected) "☑" else "☐"} ${option.label}") },
                                    onClick = {
                                        onPageFilterChange(
                                            if (selected) selectedPageFilterIds - option.id
                                            else selectedPageFilterIds + option.id
                                        )
                                    }
                                )
                            }
                            DropdownMenuItem(
                                text = { Text("清除页面筛选") },
                                onClick = { onPageFilterChange(emptySet()) }
                            )
                        }
            ...
            Box {
                OutlinedButton(onClick = { statusFilterExpanded = true }) {
                ...
                DropdownMenu(
                    ...
                    statusFilterOptions.forEach { option ->
                        val selected = option.id in selectedStatusFilterIds
                        DropdownMenuItem(
                            text = { Text("${if (selected) "☑" else "☐"} ${option.label}") },
                            onClick = {
                                onStatusFilterChange(
                                    if (selected) selectedStatusFilterIds - option.id
                                    else selectedStatusFilterIds + option.id
                                )
                            }
                        )
                    }
                    DropdownMenuItem(
                        text = { Text("清除状态筛选") },
                        onClick = { onStatusFilterChange(emptySet()) }
                    )
                }
            }
            Box {
...
@Composable
internal fun ScrollStateScrollIndicator(
    state: ScrollState,
    modifier: Modifier = Modifier
) {
    ...
    BoxWithConstraints(
        ...
        val contentPx = viewportPx + state.maxValue.toFloat()
        val visibleFraction = (viewportPx / contentPx).coerceIn(0.08f, 1f)
        val thumbHeight = (maxHeight * visibleFraction).coerceAtLeast(36.dp)
        val progress = (state.value.toFloat() / state.maxValue.toFloat()).coerceIn(0f, 1f)
        val yOffset = (maxHeight - thumbHeight) * progress
        Box(
            Modifier.fillMaxHeight().fillMaxWidth().background(
                MaterialTheme.colorScheme.onSurface.copy(alpha = 0.10f), RoundedCornerShape(99.dp)
            )
        )
        Box(
            Modifier.offset(y = yOffset).height(thumbHeight).fillMaxWidth().background(
                MaterialTheme.colorScheme.onSurface.copy(alpha = 0.45f), RoundedCornerShape(99.dp)
            )
        )
    }
}
