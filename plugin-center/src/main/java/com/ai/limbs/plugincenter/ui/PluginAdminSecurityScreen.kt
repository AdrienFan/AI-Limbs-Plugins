...
import com.ai.limbs.plugincenter.runtime.SelfMaintenanceFacade
import com.ai.limbs.plugincenter.runtime.DynamicNavigationFacade
import com.ai.limbs.plugincenter.model.PluginInactivityPolicyStore
...
internal fun PluginAdminSecurityScreen(
    ...
    adminSecurity: AdminSecurityFacade,
    selfMaintenance: SelfMaintenanceFacade,
    ...
    val upgradeLauncher = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
    ...
    LaunchedEffect(Unit) {
        ...
        inactivityTestMode = inactivity.mode == InactivityThresholdMode.TEST_SECONDS
        inactivityDays = inactivity.days.toString()
    ...
    Column(
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
            Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Text(
                    "当前版本：${maintenanceStatus.installedVersion ?: "未知"}",
                    style = MaterialTheme.typography.bodyMedium
                )
                Text(
                    "当前备份：${maintenanceStatus.currentBackupVersion ?: "无"} · 上一版本备份：${maintenanceStatus.previousBackupVersion ?: "无"}",
                    style = MaterialTheme.typography.bodySmall,
                ...
                Row(
                    ...
                    OutlinedButton(
                        onClick = {
                            scope.launch {
                                busy = true
                                runCatching { selfMaintenance.repair() }
                                    .onSuccess { returnToToolbox("Plugin Center 修复已开始，请重新打开") }
                                    .onFailure(onError)
                                busy = false
                            }
                        },
                        enabled = !busy && maintenanceStatus.canRepair,
                        modifier = Modifier.weight(1f)
                    ) { Text("修复当前") }
                    OutlinedButton(
                        onClick = {
                            scope.launch {
                                busy = true
                                runCatching { selfMaintenance.rollback() }
                                    .onSuccess { returnToToolbox("Plugin Center 回滚已开始，请重新打开") }
                                    .onFailure(onError)
                                busy = false
                            }
                        },
                        enabled = !busy && maintenanceStatus.canRollback,
                        modifier = Modifier.weight(1f)
                    ) { Text("回滚上一版") }
                }
                Text(
        ...
        if (developerMode) {
            ...
            PluginCollectionSection(
                ...
                if (filteredPrimitives.isEmpty()) {
                    ...
                    HostPrimitiveCards(filteredPrimitives, busy) { primitive, allowed ->
...
private fun HostPrimitiveCards(
    ...
    busy: Boolean,
    onToggle: (HostPrimitiveSnapshot, Boolean) -> Unit
) {
    primitives.forEach { item ->
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
            Row(
                ...
                if (item.policyAllowed != null) {
                    Checkbox(
                        checked = item.policyAllowed,
                        enabled = !busy,
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    if (definition.exposure.name == "KERNEL_GATE") {
                        Text("Kernel-owned gate · 不向普通插件开放调用", style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
        }
...
private fun surfaceKindTitle(kind: HostSurfaceKind): String = when (kind) {
    ...
    HostSurfaceKind.HOST_CAPABILITY -> "Bound Host Primitive Policies"
